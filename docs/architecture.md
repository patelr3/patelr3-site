...
| **jaeger**              | Jaeger 2             | Distributed trace collection and UI                    | 16686 |
...
| `OTEL_EXPORTER_OTLP_ENDPOINT` | OTLP endpoint for trace export (Jaeger)         |
| `OTEL_SERVICE_NAME`       | OpenTelemetry service name for this container      |
| `APPLICATIONINSIGHTS_CONNECTION_STRING` | Azure App Insights connection string   |
| `LOG_LEVEL`               | Pino log level (default: info; use silent in tests)|
...
All services use **OpenTelemetry** for distributed tracing and **pino** for structured JSON logging.

### Request Flow

```
Browser (OTel Web SDK) → Nginx (X-Request-Id, traceparent propagation)
  → Backend services (OTel Node SDK + pino)
    → Jaeger (OTLP)
...
|-----------|-------|------------|
| **Trace Collector** | Jaeger all-in-one (docker-compose) | Jaeger ACA (internal) + App Insights |
| **Trace UI** | Jaeger UI (localhost:16686) | Jaeger ACA + Azure portal |
| **Logging** | pino → stdout (pretty-printed) | pino → JSON → Log Analytics |
| **Browser Traces** | OTel Web SDK → /otlp/ → Jaeger | OTel Web SDK → /otlp/ → Jaeger |
| **AI Agent Traces** | — | App Insights → Foundry Traces tab |

### Shared Tracing Package (`packages/tracing/`)

All backend services share a local npm package that provides:
- `initTracing(serviceName)` — Initialize OTel SDK (call before any imports)
- `createLogger(serviceName)` — Create pino logger with trace context
- `requestIdMiddleware(logger)` — Express middleware for X-Request-Id propagation
- `httpLogger(logger)` — pino-http request logging middleware

### Key Headers

| Header | Purpose |
|--------|---------|
| `X-Request-Id` | Generated by nginx (`$request_id`), forwarded to all upstreams, returned in responses |
| `traceparent` | W3C Trace Context propagation (browser → nginx → services) |
| `tracestate` | W3C Trace Context state propagation |

### Foundry Agent Tracing

Application Insights is connected to the Azure AI Foundry project, enabling:
- Server-side traces for agent runs (automatic, no code changes)
- Agent Monitoring Dashboard (token usage, latency, success rates)
- Conversation tracking (trace ID → conversation history)
