...
| SunnieAI OAuth consent link not shown | `chat.js` extracting wrong field — Foundry uses `consent_link` (SSE event) and `authorization_url` (output item), NOT `url` | Check fallback chain: `consent_link \|\| authorization_url \|\| url` in chat.js |
| SunnieAI OAuth infinite loop (authorize → same consent again) | `oidc_access_tokens.token` column too narrow (VARCHAR(255)) — Foundry JWTs exceed 255 chars, INSERT fails silently | `ALTER TABLE oidc_access_tokens ALTER COLUMN token TYPE TEXT` (also oidc_refresh_tokens) |
| SunnieAI MCP `tool_user_error: 404 enumerating tools` | MCP server only handles POST /mcp — Foundry uses SSE transport (GET first to discover endpoint, then POST) | Add `GET /mcp` handler returning SSE stream with `event: endpoint\ndata: /mcp\n\n` |
...
| Foundry Bicep deploy fails "NetworkAcls required" | New Foundry experience requires `networkAcls` on CognitiveServices account | Add `networkAcls: { defaultAction: 'Allow' }` to account properties in `foundry.bicep` |
| Foundry model deploy fails for gpt-4.1 | Wrong SKU type — `Standard` not available for gpt-4.1 in most regions | Use `GlobalStandard` SKU instead of `Standard` in Bicep and parameters |
